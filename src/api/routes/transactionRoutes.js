...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await TransactionService.getTransactions(page, pageSize, orderBy, req.query.customerName);
    res.json(result);
  } catch (err) {
...
router.get('/customer/:id', async (req, res) => {
  try {
    ...
    const { page, pageSize } = getPaginationParams(req);
    const result = await TransactionService.getTransactionsByCustomer(id, page, pageSize, req.query.customerName);
    res.json(result);
  } catch (err) {
...
router.post('/', async (req, res) => {
  ...
  try {
    const customerId = await TransactionService.getNextCustomerId();
    const lastID = await TransactionService.createTransaction({
      customerId,
    ...
    await RewardsService.updateRewardsTables();
    
    LoggerService.info('Transaction created:', { customerId, transactionId: lastID });
    
    TransactionService.emitCustomerAdded({ id: lastID, customerId });
